...
|`subdomains` ✴|`string[]`|`["www"]`<br>`["@"]`<br>`["api.stage", "stage-api"]`|One or more subdomains whose DNS records you want to update, not including `domain` or a trailing period. To update `domain` itself, set this to `["@"]` (default). Can also be multi-level subdomains.|
